...
export class NotebookMarkdownRenderer {
	...
	renderMarkdown(markdown: IMarkdownString,
		...
		const renderer = rendererFactory ? rendererFactory(opts) : useNewRenderer ? new vsMarked.Renderer({ baseUrl: notebookFolder }) : new sqlMarked.Renderer({ baseUrl: notebookFolder });
		...
		if (useNewRenderer) {
			...
			element.innerHTML = parser ? parser(markdown.value, markedOptions) : vsMarked.parse(markdown.value, markedOptions);
		} else {
			...
			element.innerHTML = parser ? parser(markdown.value, markedOptions) : sqlMarked.parse(markdown.value, markedOptions);
		}
